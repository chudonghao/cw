...
FIRSTS_t FIRST(const Grammar& g, const FIRSTS_db& first_db, alpha_iter_t alpha_begin, alpha_iter_t alpha_end) {
  ...
  for (auto it = alpha_begin; it != alpha_end; ++it) {
    auto X = *it;
    // Continue into the next symbol only while this prefix can derive epsilon.
    if (g.IsTerminal(X)) {
      firsts.insert(X);
      break;
    } else {
  ...
  if (empty_count == alpha_size) {
    // The whole sequence is nullable only when every symbol is nullable.
    firsts.insert(g.ε());
  }
...
FIRSTS_db FIRSTS(const Grammar& g) {
  ...
  // Propagate through recursive productions until no FIRST set grows.
...
FOLLOWS_db FOLLOWS(const Grammar& g, const FIRSTS_db& firsts_db) {
  ...
  // FOLLOW sets depend on one another, so revisit productions until stable.
  ...
  for (; has_new_follow;) {
    ...
    for (auto& p : g.P()) {
      for (auto it = p.r.begin(); it != p.r.end(); ++it) {
        ...
        // If beta is nullable, FOLLOW(A) is also included in FOLLOW(B).
...
std::set<int> LR0_CLOSURE(const Grammar& g, const std::vector<LRItem>& lr_items, std::set<int> I) {
  // Expand nonterminals after the dot until the item set is closed.
...
LRCanonicalCollection LRk_CANONICAL_COLLECTION(const Grammar& g, const std::vector<LRItem>& lr_items,
                                               GOTO_func&& goto_func) {
...
#if 0
  std::unordered_set<std::pair<int, int>, boost::hash<std::pair<int, int>>> goto_set;
...
#else
  // Insertion extends C; the growing bound visits each newly discovered state.
  for (int ii = 0; ii < C.size(); ++ii) {
    for (auto X : g.V()) {
      auto& I = C[ii];
      auto J = goto_func(g, lr_items, I, X);
...
MultiActionLRParseTable LRk_PARSE_TABLE(const Grammar& g, const std::vector<LRItem>& lr_items,
                                        const LRCanonicalCollection& C, ReduceFunc&& reduce) {
...
MultiActionLRParseTable LR0_PARSE_TABLE(const Grammar& g, const std::vector<LRItem>& lr_items,
                                        const LRCanonicalCollection& C) {
  return LRk_PARSE_TABLE(g, lr_items, C, LR0ReduceFunc());
}

MultiActionLRParseTable SLR_PARSE_TABLE(const Grammar& g, const FOLLOWS_db& follows_db,
                                        const std::vector<LRItem>& lr_items, const LRCanonicalCollection& C) {
